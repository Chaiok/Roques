...
(defn collision [player px py radius]
  (let [
      ...
      f (atom 0)
      color (get (@player/streams player) "color:") 
      ochki (get (@player/streams player) "ochki:")
      f1 (atom color)
    ]
    (if ( and (> ochki 10) (< ochki 20))
      (reset! f1 1)
        (if ( and (> ochki 19) (< ochki 30))
          (reset! f1 2)
        )
    )
    (doseq [[k v] players]
    ...
    (doseq [[k v] (@player/streams "block:")]
      (let [
        ...
        (if (< (+ (* (- px x) (- px x)) (* (- py y) (- py y))) (* (+ radius radius) (+ radius radius)))
          (if (< px 40 )
            (commute player/streams assoc player {"x:" (+ px 100) "y:" py "ochki:" (get (@player/colorOchki player ) :ochki) "color:" @f1 })
            (if (> px 760)
              (commute player/streams assoc player {"x:" (- px 100) "y:" py "ochki:" (get (@player/colorOchki player ) :ochki) "color:" @f1 })
              (if (> py 560)
                (commute player/streams assoc player {"x:" px "y:" (- py 100) "ochki:" (get (@player/colorOchki player ) :ochki) "color:" @f1 })
                (commute player/streams assoc player {"x:" px "y:" (+ py 100) "ochki:" (get (@player/colorOchki player ) :ochki) "color:" @f1 })
              )
            )
          )
    ...
    (if (= @f 0)
      ...
      (commute player/streams assoc player {"x:" px "y:" py "ochki:" (get (@player/colorOchki player ) :ochki) "color:" @f1 })
    )
  )
)
